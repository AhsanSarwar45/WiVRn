...
void settings_tracking(const settings_context & ctx)
{
	auto & config = ctx.config;
	const std::string disconnect_tip = ctx.in_game ? _("Disconnect to change this setting.") : std::string{};
	std::vector<setting> list;
	...
	if (body_tracker != xr::body_tracker_type::none)
	{
		list.push_back({
		        ...
		        .description = body_tracker == xr::body_tracker_type::fb
		...
		if (body_tracker == xr::body_tracker_type::fb)
		{
			list.push_back({
			        .id = "##lower_body",
			        .label = _("Lower body tracking"),
			        .description = _("Estimate lower body joint positions using Generative Legs."),
			        .ui = ui_kind::toggle,
			        .get_bool = [&config] { return config.fb_lower_body; },
			        .set_bool = [&config](bool v) { config.fb_lower_body = v; config.save(); },
			        .default_bool = false,
			        .enabled = [&ctx, &config] { return not ctx.in_game and config.check_feature(feature::body_tracking); },
			        .disabled_tooltip = disconnect_tip,
			});
			list.push_back({
			        .id = "##hip",
			        .label = _("Hip tracking"),
			        .description = _("Only takes affect with lower body tracking enabled. May be desired when using another source of hip tracking."),
			        .ui = ui_kind::toggle,
			        .get_bool = [&config] { return config.fb_hip; },
			        .set_bool = [&config](bool v) { config.fb_hip = v; config.save(); },
			        .default_bool = true,
			        .enabled = [&ctx, &config] { return not ctx.in_game and config.check_feature(feature::body_tracking) and config.fb_lower_body; },
			        .disabled_tooltip = disconnect_tip,
			});
		}
	}
