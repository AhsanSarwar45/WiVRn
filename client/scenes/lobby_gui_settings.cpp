	...
	if (body_tracker != xr::body_tracker_type::none)
	{
		...
		if (body_tracker == xr::body_tracker_type::fb)
		{
			list.push_back({
			        ...
			        .default_bool = false,
			        .enabled = [&config] { return config.check_feature(feature::body_tracking); },
			});
			list.push_back({
			        .id = "##hip",
			        ...
			        .default_bool = true,
			        .enabled = [&config] { return config.check_feature(feature::body_tracking) and config.fb_lower_body; },
			});
		}
	}
